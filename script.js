...
`;
      ...
      if (installBtn) {
        installBtn.addEventListener('click', async function () {
          ...
          if (!deferredInstallPrompt) {
            showToast('ℹ️ 请使用浏览器菜单中的“安装应用/添加到桌面”', 2800, true);
            return;
          }
          try {
        ...
        window.addEventListener('load', function () {
          navigator.serviceWorker.register('/service-worker.js', { scope: '/' })
            .then(function (registration) {
              ...
              console.log('[PWA] Service Worker 已注册，scope=', registration.scope);
            })
  ...
  const state = {
    ...
    r2Config: {},     // 来自 /api/env 的 cloudflare_r2 配置（不含任何前端内置值）
    publicMessages: [],
  ...
  async function lookupAvatarUrlFromStorageBucket(userId) {
    ...
    if (_avatarBucketLookupMissUntil[id] && Date.now() < _avatarBucketLookupMissUntil[id]) return '';
    if (_avatarBucketLookupInFlight.has(id)) return _avatarBucketLookupInFlight.get(id);
    const promise = (async function () {
      try {
        const response = await fetch('/api/avatar?user_id=' + encodeURIComponent(id) + '&_=' + Date.now(), {
          method: 'GET', headers: { 'Accept': 'application/json' }, cache: 'no-store'
        });
        const data = await response.json().catch(() => ({}));
        if (response.ok && data && data.ok && data.exists && data.url) {
          const url = String(data.url);
          _avatarBucketLookupCache[id] = { url: url, expiresAt: Date.now() + 60 * 1000 };
          return url;
        }
      } catch (e) {
  ...
  function syncAvatarFromStorageBucket(userId, onDone) {
    const id = String(userId || '').trim();
    lookupAvatarUrlFromStorageBucket(id).then(function (url) {
  ...
  function syncMissingMemberAvatarsFromBucket() {
    (state.onlineMembers || []).forEach(function (member) {
      ...
      lookupAvatarUrlFromStorageBucket(id).then(function (url) {
        if (!url) return;
        const target = (state.onlineMembers || []).find(function (m) { return m && String(m.id) === id; });
        ...
        updateOnlineMembersUI();
        state.servers.forEach(function (server) { renderChatMessages(server.id, false); });
        renderPublicChat(false);
      }).catch(function () {});
    });
  }
  ...
  function saveUserId(newId) {
    ...
    if (!/^https?:\/\//i.test(String(state.avatar || ''))) state._pendingAvatarSync = true;
    // 无论本地是否有旧 URL，都使用稳定 userId 从 R2 校准到最新头像。
    setTimeout(function () { syncAvatarFromStorageBucket(trimmed); }, 0);
    // 更新本地成员列表中的自己
  ...
  function _broadcastPresenceSelf() {
    ...
    const avatar = /^https?:\/\//i.test(String(state.avatar || '')) ? String(state.avatar) : '';
    // 自己也写入资料缓存
    ...
    function trySave() {
      ...
      if (showUserId && idInput) {
        ...
        try {
          ...
          if (restored && /^https?:\/\//i.test(String(restored.avatar || ''))) {
        ...
        setTimeout(function () { syncAvatarFromStorageBucket(newId); }, 0);
  ...
  function updateChatUI() {
    ...
    document.querySelectorAll('.chat-plus-btn, .chat-voice-btn').forEach(btn => {
      btn.disabled = !ready;
    });
  ...
  function _fallbackPublicAccessByHost() {
    let host = '';
    try { host = String(window.location.hostname || '').trim().toLowerCase(); } catch (_) {}
    if (!host || host === 'localhost' || host === '::1' || host.endsWith('.local') || host.endsWith('.lan')) return false;
  ...
  async function _isPublicDownloadAccess() {
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (response.ok && data && data.ok === true && typeof data.is_public === 'boolean') {
  ...
  async function _browserDownload(url, fileName, isXor, mimeType) {
    ...
    try {
      ...
      showToast('❌ 浏览器下载失败：' + (e && e.message ? e.message : e), 3500, false);
      return false;
    }
  }
  ...
  function sendMessageWithMedia(serverId, inputElement, sendFunction, isPublic, accept) {
    ...
    fileInput.onchange = async (e) => {
      ...
      if (oversized.length) {
        showToast('❌ ' + oversized.length + ' 个文件超过 ' + maxUploadMb + 'MB 已跳过', 2500, false);
      }
      ...
      try {
        ...
        if (!_appkey || !_host) {
          ...
          if (!_goEasyMissingConfigToastShown) {
            ...
            showToast('⚠️ GoEasy 未配置，请先在环境变量配置中填写 AppKey 和主机', 3200, false);
          }
          return;
        }
        _goEasyMissingConfigToastShown = false;
        const _forceTLS = (state.goEasyConfig && typeof state.goEasyConfig.force_tls === 'boolean')
          ? state.goEasyConfig.force_tls : true;
        ...
        const presenceAvatar = /^https?:\/\//i.test(String(state.avatar || '')) ? String(state.avatar) : '';
        goEasy.connect({
          id: userId,
          ...
          onSuccess: function () {
            ...
            try {
              if (state._pendingAvatarSync || !/^https?:\/\//i.test(String(state.avatar || getStoredAvatar() || ''))) {
  ...
  async function uploadAvatarToStorageBucket(blob, userId) {
    ...
    _avatarBucketLookupCache[id] = { url: String(data.url), expiresAt: Date.now() + 60 * 1000 };
  ...
  function openAvatarCropModal(dataUrl, fileName) {
    ...
    modal.querySelector('#avatarCropOkBtn').addEventListener('click', async function () {
      try {
        ...
        const userId = String(state.userId || getStoredUserId() || '').trim();
        const avatarUrl = await uploadAvatarToStorageBucket(avatarBlob, userId);
  ...
  if (state.userId) {
    ...
    setTimeout(function () { syncAvatarFromStorageBucket(state.userId); }, 0);
  }
  loadMemberProfiles();
  // 自己的最新资料写入缓存；旧 base64 不再进入成员同步缓存。
  if (state.userId) {
    const cachedAvatarUrl = /^https?:\/\//i.test(String(state.avatar || '')) ? state.avatar : '';
  ...
  function _showEnvSecurityModal(title, bodyHtml, submitText, onSubmit) {
    ...
    modal.innerHTML =
      '<div class="custom-modal-box" style="width:min(380px,calc(100% - 32px));">' +
        '<div class="custom-modal-header"><span>' + esc(title) + '</span>' +
          '<button type="button" class="custom-modal-close" aria-label="关闭">✕</button></div>' +
        '<div class="custom-modal-body">' +
          '<div class="form-grid">' + bodyHtml +
          ...
          '<button type="button" id="envSecurityOkBtn" class="submit-btn" style="margin-top:14px;">' +
            '<span class="spinner"></span><span class="btn-text">' + esc(submitText) + '</span></button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(modal);
    ...
    function setLoading(on) {
      ...
      if (okBtn) { okBtn.disabled = on; okBtn.classList.toggle('loading', !!on); }
    }
    ...
    modal.querySelector('#envSecurityOkBtn').addEventListener('click', async function () {
  ...
  function forceSetPassword(onDone) {
    _showEnvSecurityModal(
      ...
      async function (modal, setLoading, showErr, close) {
        ...
        if (!p1 || p1.length < 4) { showErr('安全密码至少需要 4 位'); return; }
        if (p1 !== p2) { showErr('两次输入的密码不一致，请重新确认'); return; }
  ...
  function requirePasswordToOpen(onDone) {
    _showEnvSecurityModal(
      ...
      async function (modal, setLoading, showErr, close) {
        ...
        if (!d.verified) { showErr('密码错误，请重试'); return; }
  ...
  function bindEnvSettingsEvents() {
    ...
    function open() {
      ...
      if (pathEl) pathEl.textContent = '配置文件：' + (state.envConfigFile || '… 加载中');
      loadEnvIntoForm();
    }
    function close() {
      modal.classList.remove('open');
    ...
    async function loadEnvIntoForm() {
      ...
      try {
        ...
        const go = (d.config.goeasy && typeof d.config.goeasy === 'object') ? d.config.goeasy : {};
        setVal('envGoEasyAppkey', go.appkey);
        setVal('envGoEasyHost', go.host);
        const r2 = (d.config.cloudflare_r2 && typeof d.config.cloudflare_r2 === 'object') ? d.config.cloudflare_r2 : {};
    ...
    async function save() {
      ...
      const payload = {
        ...
        cloudflare_r2: {
          ...
          max_upload_mb: val('envR2MaxUploadMb').trim() ? parseInt(val('envR2MaxUploadMb'), 10) : '',
          max_storage_mb: val('envR2MaxStorageMb').trim() ? parseInt(val('envR2MaxStorageMb'), 10) : '',
      ...
      try {
        ...
        if (d.config && d.config.goeasy && typeof d.config.goeasy === 'object') {
          state.goEasyConfig = d.config.goeasy;
        }
        if (d.config && d.config.cloudflare_r2 && typeof d.config.cloudflare_r2 === 'object') {
  ...
  loadEnvConfig().finally(async () => {
    ...
    try { await migrateLegacyAvatarToStorageBucket(); } catch (e) {}
    ensureGoEasySdk(() => {
